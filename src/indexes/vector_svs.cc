...
absl::StatusOr<std::shared_ptr<VectorSVS<T>>> VectorSVS<T>::Create(
  ...
  if (IsLeanVecCompression(config.compression)) {
    status = svs::runtime::v0::DynamicVamanaIndexLeanVec::build(
        ...
        storage_kind, config.leanvec_dims, build_params, search_params,
        dyn_params);
  } else if (dyn_params.deferred_compression_threshold > 0) {
    status = svs::runtime::v0::DynamicVamanaIndex::build(
        &index->svs_index_, vector_index_proto.dimension_count(), svs_metric,
        storage_kind, build_params, search_params, dyn_params);
  } else {
    status = svs::runtime::v0::DynamicVamanaIndex::build(
        &index->svs_index_, vector_index_proto.dimension_count(), svs_metric,
...
void VectorSVS<T>::UpdateReportedMemory() {
  // get_memory_usage() is available in the svs-memory-reporting nightly
  // (2026-08-17) but not in the deferred-compression nightly (2026-06-23).
  // Until both APIs land in a single release this function is a no-op on
  // the deferred-compression branch.
#if SVS_HAS_GET_MEMORY_USAGE
