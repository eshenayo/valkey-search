...
static int run_case(const char* label, size_t dim, size_t n, bool incremental) {
  ...
  if (incremental) {
    ...
    if (!st.ok()) { fail("add-bulk", st.message()); DVamana::destroy(idx); return 1; }
  ...
  st = reloaded->search(1, q.data(), 10, dists.data(),
                        result_labels.data(), nullptr, nullptr);
